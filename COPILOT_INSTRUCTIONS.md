...
3. Confirm with the user:  
   ...
   ## Environment & available tools (DEVCONTAINER)
   This workspace is designed to run inside a devcontainer on **Ubuntu 24.04.2 LTS**. The following CLI tools are commonly available on PATH in the container and safe for agent use:
   ...
   Browser helper
   - Use the host browser from the devcontainer with:
     - `$BROWSER <url>`
     - Example: `$BROWSER https://github.com`

   Rule: Honour user-provided coding instructions
   - If a user supplies repo-specific coding instructions (e.g., available tools, environment, or required flows), prefer them when generating code or running commands, unless they directly contradict higher-priority system policies.
   - Do not run destructive operations without explicit confirmation.
   - Prefer non-destructive flows by default (use `--defaults`, temporary directories, and request confirmation before commits/PRs).
